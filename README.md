...
- **AI / Composer** (`includes/ai/`, `includes/admin/class-ai.php`) — richiede una chiave OpenAI (§5.3, §6):
  ...
  - **OpenAI Storage + File Search + media** (`ai/class-openai.php`, `ai/class-composer.php`): il client supporta **Responses API con File Search** (`vector_store_ids`), l'**upload file** e i **vector store**. `Composer::build_from_sources()` interroga i **documenti del progetto su OpenAI Storage** (File Search) e sceglie le immagini dai **media del sito** per **popolare i campi strutturati** (`_pll_editorial` + meta) con i contenuti corretti; `Composer::upload_documents()` carica i documenti allegati (PDF/testo) sul **vector store** configurato in **Palladio → AI**. `Composer::apply_structured()` scrive meta ed editoriale con **aggiornamenti parziali** (merge sull'esistente).
- **Agente Studio** (`includes/admin/class-studio.php`) — agente di regia consapevole dell'**intera struttura** e dello **Storage**:
  - Pagina **Palladio → Agente AI** (chat) e pulsante **“🪄 Avvia l’agente AI”** nel metabox AI di edifici/unità.
  - **Consapevolezza completa** via tool: `get_structure` (tutti gli edifici con unità e scenari + dati chiave), `get_entity`, `list_media`, `search_project_documents` (File Search sui documenti su Storage).
  - **Popolamento contenuti**: `update_entity` (title/excerpt/content, meta, campi editoriali — aggiornamenti parziali), `create_unit`, `create_scenario`.
  - Loop di function calling server-side (nessuna chiamata AI dal browser); nonce + capability `manage_palladio`; scritture protette da `edit_post`. Non inventa dati: prezzi/misure dai documenti, immagini solo dai media del sito.
